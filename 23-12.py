...
def extract_text(path: str, page_index: int) -> str:
    ...
    if pdf_text.strip() and not is_bad_pdf_text(pdf_text):
        ...
        text = ocr_text
        print("[TEXT SOURCE] OCR ONLY (forced)", file=sys.stderr)
...
def extract_account_number(text: str, form_type: str = "", page_number: int = None) -> str | None:
    ...
    if is_1099r_page_for_skip(text):
        if page_number is not None:
            print(f"⛔ Page {page_number} → 1099-R detected, skipping account extraction")
        return None
    # ==========================================================
...
def extract_1098t_bookmark(text: str) -> str:
    ...
    KEYWORDS = r"(University|College|Institute|Academy|Board of Regents)"
