...
def k1_page_priority(text):
    ...
    if "supplemental information" in t:
        return 4
    if "statement a" in t or "section 199a" in t or "qbi" in t:
        return 5
    if "rental real estate" in t:
        return 6
    if "basis worksheet" in t or "at-risk basis" in t:
        return 7
    if "continuation" in t:
        return 8
    return 99

def extract_k1_ein(text):
    m = re.search(r"\b\d{2}-\d{7}\b", text)
    return m.group(0) if m else None
def extract_k1_company(text):
    clean = re.sub(r"[^\w\s,&.'\-]", " ", text)
    clean = re.sub(r"\s+", " ", clean)

    m = re.findall(
        r"[A-Z][A-Z\s,&.'\-]{3,80}?(?:LLC|INC|CORP|LP|LLP|LLLP|FUND)",
...
def extract_k1_company(text: str) -> str | None:
    """
    Extracts the partnership/entity name from K-1 pages.
    Works for 1065, 1120-S, 1041 versions.
    Avoids IRS instructions and continues pages.
    """
    ...
    # Remove lines that are obviously IRS instructions
    bad_phrases = [
        "negligence penalty", 
        "may be imposed",
        "attach this k-1 to",
    ...
    # Look for company name before address block (OCR-safe)
    for i, line in enumerate(cleaned):
        # Skip lines containing numbers (addresses)
        if re.search(r"\d{2,}", line):
            continue

        # Accept company/entity name-like text
        if len(line) >= 4 and not re.search(r"^[0-9\s]+$", line):
...
def merge_with_bookmarks(input_dir: str, output_pdf: str):
   ...
   # remove any zero‐byte files so PdfReader never sees them
    ...
    income, expenses, others = [], [], []    # ---- Consolidated-1099 synthesis (insert this BEFORE income.sort(...)) ----
    ...
    account_names = {}
    k1_pages = {}       # {EIN: [(path, page_index)]}
    k1_names = {}       # {EIN: partnership name}
    k1_form_type = {}   # {EIN: '1065'/'1120S'/'1041'}
    ...
    for fname in files:
        ...
        if fname.lower().endswith('.pdf'):
            ...
            for i in range(total):
                ...
                # 🚫 Never run K-1 detection on consolidated pages
    ...
    # 🚫 DO NOT mark consolidated yet — wait until grouping step
                    
                if (path, i) not in consolidated_pages:

                    lower = tiered.lower()

                    # Detect K-1 related pages
                    is_k1 = (
                        "schedule k-1" in lower or
                        "form 1065" in lower or
                        "form 1120-s" in lower or
                        "form 1041" in lower or
                        "statement a" in lower or
                        #("section 199a" in lower and "schedule k-1" in lower)
                        "qualified business income" in lower or
                        "supplemental information" in lower or
                        "basis worksheet" in lower or
                        "continuation" in lower
                    )

                    if is_k1:
                    # Extract EIN (fallback for continuation pages)
                        ein = extract_k1_ein(tiered)
                        if not ein:
                            ein = f"NOEIN_{os.path.splitext(fname)[0]}"

                    # Store pages grouped by EIN
                        k1_pages.setdefault(ein, []).append((path, i))

                    # Detect 1065 / 1120S / 1041
                        if "form 1065" in lower:
                            k1_form_type[ein] = "1065"
                        elif "form 1120-s" in lower:
                            k1_form_type[ein] = "1120S"
                        elif "form 1041" in lower:
                            k1_form_type[ein] = "1041"
                        else:
                            k1_form_type.setdefault(ein, "1065")

        # Extract company/partnership name
                        comp = extract_k1_company(tiered)
                        if comp:
                            k1_names[ein] = comp               
                ...
                # 🚫 Skip reclassification if this page already belongs to Consolidated-1099
                if (path, i) in consolidated_pages:
                    print(f"[SKIP] {os.path.basename(path)} Page {i+1} already under Consolidated-1099 – skipping other classification", file=sys.stderr)
                    continue
...
# --------------------------------------------------------------------------
    # ---- Build unified K-1 groups ----
    k1_payload = {}     # synthetic group for merge phase
    ...
    for ein, pages in k1_pages.items():
        ...
        key = f"K1::{k1_form_type.get(ein,'1065')}::{ein}"

        k1_payload[key] = [(p, idx, "K-1") for (p, idx) in pages_sorted]

        # Add single synthetic row into income
    ...
    def append_and_bookmark(entry, parent, title, with_bookmark=True):
        ...
        # ⭐ Handle synthetic K-1 group entries
        if isinstance(entry[0], str) and entry[0].startswith("K1::"):
            key = entry[0]
            if key in k1_payload:
                for (rp, ridx, _) in k1_payload[key]:
                    append_and_bookmark((rp, ridx, "K-1"), parent, title, with_bookmark=False)
            return
...
# Append once, with or without bookmark
    ...
    # Only zero INT → keep page, no bookmark
                ...
                continue
            if form == 'K-1':
                k1_root = merger.add_outline_item("K-1 Packages", page_num, parent=root)

                for key, items in k1_payload.items():
                    _, formtype, ein = key.split("::")
                    comp = k1_names.get(ein, f"EIN {ein}")

                # anchor page
                    anchor_path, anchor_idx, _ = items[0]
                    anchor_page = page_num

                    pkg_node = merger.add_outline_item(f"{comp} (Form {formtype})", anchor_page, parent=k1_root)

                    for (p2, i2, _) in items:
                        append_and_bookmark((p2, i2, 'K-1'), pkg_node, "", with_bookmark=False)
