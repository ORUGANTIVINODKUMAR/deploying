...
def is_unused_page(text: str) -> bool:
    ...
    return (
        ...
        or "allocated based on information provided by the security issuer" in norm  
        or "importanttaxreturndocumentenclosed" in norm
        or "may show an account or other unique number the payer assigned to" in norm
        or "shows teh market discount that accrued on the debt instrument during the year" in norm
        or "may also show the aggredate amount of cash and their market" in norm
        #Apex clearing
...
def con_unused_page(text: str) -> bool:
    """
    Detect pages that are just year-end messages, instructions,
    or generic investment details (not real 1099 forms).
    """
...
def extract_account_number(text: str, page_number: int = None) -> str:
    ...
    lower = text.lower()
    # --- NEW: Skip if unused page ---
    if con_unused_page(lower):
        if page_number is not None:
            print(f"⛔ Page {page_number} → UNUSED page detected, skipping account number")
        return None
    ...
    for fname in files:
        ...
        if fname.lower().endswith('.pdf'):
            ...
            for i in range(total):
                ...
                # Condition: page looks like a consolidated 1099 statement
                if acct_num and "1099" in lower_text and not re.search(r"1098[-\s]*t", lower_text, re.IGNORECASE):

                    account_pages.setdefault(acct_num, []).append((path, i, "Consolidated-1099"))

                    print(f"[EARLY EXIT] Consolidated-1099 detected → {os.path.basename(path)} p{i+1}", file=sys.stderr)

                    # Mark page for consolidated processing ONLY
                    ...
                    # Also add real page entry to the payload
                    consolidated_payload.setdefault(synthetic_key, []).append((path, i, "Consolidated-1099"))
                    # Mark this page as consolidated immediately
                    consolidated_pages.add((path, i))


                    # DO NOT allow further processing (prevents K-1, EIN, W-2, etc.)
                    continue
