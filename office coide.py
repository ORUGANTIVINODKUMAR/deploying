...
def extract_account_number(text: str, page_number: int = None) -> str:
    ...
    std_patterns = [
            # Handles: SethiAccount Number: 488..., AccountNumber: 488..., Account   Number 488...
        r"[A-Za-z]*Account\s*Number\s*[:\-]?\s*([0-9][0-9\s]{5,})",

        # Also catch messy versions of "AccountNumber" stuck together
        r"AccountNumber\s*[:\-]?\s*([0-9][0-9\s]{5,})",

        # Super‐forgiving fallback (looks for any run of 9–18 digits spanned by spaces)
        r"([0-9]{3}\s+[0-9]{3,6}\s+[0-9]{2,6})",
...
def extract_consolidated_issuer(text: str) -> str | None:
    ...
    # 3️⃣ Fallback – catch lines that LOOK like company names
    for line in text.splitlines():
        ...
        if re.search(r"(LLC|Securities|Bank|Wealth|Advisors|Brokerage|Inc)", s):
            return s
    ...
    for fname in files:
        ...
        if fname.lower().endswith('.pdf'):
            ...
            for i in range(total):
                ...
                # Always store issuer the FIRST time we see this account
                if acct_num:
                    if acct_num not in account_names:
                        account_names[acct_num] = issuer if issuer else None
                    else:
                        # If later pages have no issuer, inherit stored issuer
                        if issuer is None:
