...
def extract_text(path: str, page_index: int) -> str:
    ...
    # -----------------------
    # THREAD 3 → TESSERACT (fallback)
    # -----------------------
    def do_tesseract():
        nonlocal tess_text
        try:
            img = render_page_for_ocr(path, page_index, dpi=150)
    ...
    t3 = threading.Thread(target=do_tesseract)
    ...
    text_stats("Tesseract", tess_text)
    ...
    elif tess_text.strip():
        text = tess_text
        print("[TEXT SOURCE] TESSERACT (fallback)", file=sys.stderr)
