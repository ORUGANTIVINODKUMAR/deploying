...
def extract_account_number(text: str, page_number: int = None) -> str:
    ...
    # --- 0️⃣ Normalize common OCR problems ---
    # Remove unicode lookalike characters (Cyrillic o, u, spaces, etc.)
    ...
    # --- 1️⃣ Skip extraction for forms that should not have account numbers ---
    skip_keywords = [
        "form 1099-r", "1099 r",
    ...
    # --- 2️⃣ Standard patterns (your exact logic) ---
    std_patterns = [
...
EIN_SKIP_KEYWORDS = [
    ...
    # --- Consolidated 1099 main indicators ---
    "consolidated 1099",
    "composite 1099",
    "tax information",
    ...
    # --- Boxes on consolidated pages ---
    "dividends and distributions",
    "miscellaneous information",
    "section 1256 contracts",

    # --- Brokerage TIN indicators ---
    "payer's tin",
    "recipient's tin",
...
def extract_ein_number(text: str) -> str | None:
    """
    Extract EIN from text with skip-logic and OCR-safe dash normalization.
    EIN extraction is skipped if page contains 1098, W-2, tuition statements, etc.
    """
    ...
    # --- 0️⃣ Skip EIN extraction for forms that should NOT have EIN ---
    for k in EIN_SKIP_KEYWORDS:
        if k in lower:
            # Optional: print debug
            # print(f"⛔ EIN skipped (detected keyword: '{k}')")
            return None

    # --- Normalize dash variants ---
    normalized = re.sub(r"[–—−‐‒﹘﹣]", "-", text)

    # --- 1️⃣ Standard EIN with dash ---
    m = re.search(r"\b\d{2}-\d{7}\b", normalized)
    if m:
        return m.group(0)

    # --- 2️⃣ Continuous 9 digits -> convert to EIN ---
...
def classify_text(text: str) -> Tuple[str, str]:
    ...
    if any(
        ...
        for kw in [
            ...
            "section 199a income",      # true K-1 usage
            "section 199a information",
            "199a statement a",         # used on K-1 attachments
            "199a supplemental",        # used on K-1s
            "qbi pass-through",
            "qbi pass through",
            "partnership",
...
# ============================================================
# 1) 🔥 HARD BLOCK — DETECT CONSOLIDATED-1099 FIRST
#    If TRUE → stop K-1 logic completely
# ============================================================
                ...
                if acct_num:
                    if (
                        "1099" in text_lower
                        and not re.search(r"1098[-\s]*t", text_lower)
                        and (
                            "1099-div" in text_lower
                            or "1099-int" in text_lower
                            or "1099-b"  in text_lower
                            or "summary of proceeds" in text_lower
                            or "dividends and distributions" in text_lower
                            or "consolidated 1099" in text_lower
                            or "composite 1099" in text_lower
                        )
                    ):
        # Mark page as consolidated 1099
                        ...
                        print(f"[DEBUG] {fname} p{i+1}: Consolidated-1099 detected — blocking K-1", file=sys.stderr)
                    ...
                    # BLOCK K-1
                        ...
                        is_1120s = False
                        is_1041 = False
                        ein_num = None  # ⭐ CRITICAL — prevents K-1 grouping

                        continue   # ⭐ DO NOT PROCESS THIS PAGE FURTHER

                # ✅ Detect any K-1-related page (expanded keyword list)
                # Convert entire page into list of clean lines
                lines = [line.strip().lower() for line in text.splitlines() if line.strip()]

        # K-1 full-phrase matches only
                k1_full_phrases = [
                    "form 1065",
                    "form 1120-s",
                    "form 1041",
                #    "statement a",
                    ...
                    "additional information from schedule k-1",
                    "passive activity purposes",
                ]

                def full_phrase_match(lines, phrase):
                    """Return True only if the full phrase appears in a line, not substring inside other text."""
                    phrase = phrase.lower().strip()
                    return any(phrase == line or phrase in line for line in lines)

                is_k1_page = any(full_phrase_match(lines, phrase) for phrase in k1_full_phrases)
                ...
                # --- ❌ K-1 SKIP SECTION (PREVENT FALSE POSITIVES) ---
                k1_skip_keywords = [
                    "1099",
                    "1099-div",
                    "1099-int",
                    ...
                    "consolidated 1099",
                    "tax statement",
                    "supplemental tax statement"
                ]

                if any(k in text_lower for k in k1_skip_keywords):
                    is_k1_page = False
                    is_1065 = False
                    is_1120s = False
                    is_1041 = False
...
# ============================================================
                ...
                if (is_1065 or is_1120s or is_1041 or is_k1_page) and ein_num:
                    ...
                    print(
                        f"[DEBUG] Unified K-1 page: {fname} p{i+1} EIN={ein_num} Company={company}",
                        file=sys.stderr
                    )
