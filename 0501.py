...
def extract_text(path: str, page_index: int) -> str:
    ...
    def do_ocr():
        nonlocal ocr_text
        try:
            img = render_page_for_ocr(path, page_index, dpi=240)
            gray = img.convert("L")
    ...
    # TEXT COUNT METRICS ✅
    # -----------------------
    def text_stats(label: str, t: str):
        chars = len(t)
        lines = t.count("\n")
        words = len(t.split())
        print(
            f"[TEXT COUNT] {label}: chars={chars}, lines={lines}, words={words}",
            file=sys.stderr
        )

    text_stats("PDFMiner", pdf_text)
    text_stats("OCR", ocr_text)

    # -----------------------
    # FINAL SELECTION (PRIORITY-BASED)
    # -----------------------
    if pdf_text.strip() and not is_bad_pdf_text(pdf_text):
        text = pdf_text + "\n" + ocr_text
...
def pre_classify_skip_page(text: str) -> bool:
    ...
    if (
        ...
        return True
    #Charles schwab Consolidated-1099
    if (
        "tax questions for individuals" in t
        and "tax questions for businesses" in t
        and "to order tax forms or publications" in t
        and "to pay taxes by credit card" in t
        and "for additional information and to print forms" in t
    ):
        return True
    if (
        "notes for your form 1099-B" in t
        and "detail information of dividends and distributions" in t
        and "detail information of interest Income" in t
        and "summary of fees & expenses" in t
        and "terms and conditions" in t
    ):
        return True
    if (
        "may show an account or other unique number" in t
        and "you may be able to report the amounts shown" in t
        and "be subject to an exclusion" in t
        and "foreign persons and entities" in t
        and "Any amount received in excess" in t
        
        and "Shows the portion of the amount in box 1a" in t
        and "Show cash and noncash liquidation" in t
        and "nominee return to show amounts owned by the" in t
    ):
        return True
    
    if (
        "stock has had a reportable change in" in t
        and "its account reporting requirement under chapter" in t

        and "by any commissions or transfer taxes related" in t

        and "a noncompensatory option granted or acquired" in t

        and "Any amount received in excess" in t
        
        and "any amount related to the option that was" in t

        and "proceeds are from a transaction involving" in t

        and "such as legislation enacted after they were" in t
    ):
        return True
    if (
        "acquisition date for the individual lot in the transaction" in t
        and "schwab has provided cost basis information whenever" in t
        and "schwab is not providing cost basis" in t
        and "the security was purchased more" in t
    ):
        return True
    if (
        "additional information when completing your tax" in t
        and "in your account reallocated certain income" in t
        and "please be aware that schwab uses the" in t
        and "section provides information for all your" in t
        and "If you have a complaint regarding your" in t

    ):
        return True
    if (
        "reduced by any commissions or transfer taxes" in t
        and "amount related to the option that was" in t
        and "gain on a contingent payment debt instrument" in t
        and "this amount on your income tax return" in t
        and "the exercise of a noncompensatory option" in t
        and "you may be able to report your transaction" in t
        and "section 1256 option contracts closed" in t
        and "are all used to figure the aggregate profit" in t
    ):
        return True
    if (
        "reduced by any commissions or transfer" in t
        and "proceeds from regulated futures contracts" in t
        and "basis to reflect your option premium" in t
        and "there are any special rules or adjustments" in t
        and "stock in most mutual funds purchased" in t
        and "fair market value of any property" in t
    ):
        return True
    if (
        "may show an account or other unique number" in t
        and "a nominee return to show amounts owned by" in t
        and "had a reportable change in control or capital" in t
        and "appropriate description may be shown" in t
        and "may also show the aggregate amount" in t
        and "If the securities were acquired through" in t
        and "gain on a contingent payment debt instrument" in t

    ):
        return True
    if (
        "interest paid to you must be reported" in t
        and "interest paid to you must be taken into" in t
        and " account in computing the amt reported" in t

    ):
        return True
    if (
        "consolidated 1099 sections" in t
        and "summary information" in t
        and "proceeds from broker and barter exchange" in t
        and "your account did not receive the following" in t
        and "investment details" in t

    ):
        return True
    
    #Morgan Stanley Consolidated-1099


    if (
        "brokers and barter exchanges must report proceeds from" in t
        and "show an account or other unique number the" in t
        and "item or service for which amounts are being" in t
        and "if the securities sold were acquired on a variety of" in t
        and "were acquired through the exercise of a noncompensatory option" in t
        and "exercise of a noncompensatory option resulted in a sale" in t
        and "such as legislation enacted after they were published" in t
        and "and direct deposit or payment options" in t
    ):
        return True
    
    #Robinhood Consolidated-1099
    if (
        "may show an account or other unique number the payer assigned to" in t
        and "shows your share of expenses of a nonpublicly" in t
        and "this amount may be subject to backup withholding" in t
        and "shows the cost or other basis of securities sold" in t
        and "gain on a contingent payment debt instrument subject" in t
        and "such as legislation enacted after they were" in t
    ):
        return True
    #Ameritrade Consolidated-1099
    if (
        "shows the cost or other basis of securities sold" in t
        and "shows the amount of accrued market discount" in t
        and "noncontingent bond method generally is treated as" in t
        and "and securities futures contracts entered into" in t
        and "reportable change in control or capital structure reported" in t
        and "such as legislation enacted after they were published" in t
    ):
        return True
    #Acorns securities
    if "tax character of the distribution has been allocated based on information provided by the security issuer" in t:
        return True
    if "the tax character of the distribution has" in t and "based on information provided by the security issuer" in t:
        return True
    
    if (
        "report this amount on your income tax return as tax" in t
        and "total capital gain distributions from a regulated investment" in t
        and "apply only to foreign persons and entities whose income" in t
        and "shows your share of expenses of a nonpublicly" in t
        and "satisfy its account reporting requirement under chapter" in t
        and "and direct deposit or payment options" in t
    ):
        return True
    #Etrdae consolidtaed 
    ...
    if "amortization, accretion, and similar adjustments to cost basis are not provided for short-term instruments and unit investment trusts" in t:
        return True
    if (
        "cost or other basis provided may include adjustments" in t
        and "dividend reinvestment" in t
        and "return of capital" in t
        and "wash sale loss disallowed" in t
        and "amortization" in t
        and "accretion" in t
        and "acquisition" in t
        and "option premium" in t
    ):
        return True
    #fundrise
...
def extract_account_number(
    ...
    # ==================================================
    m = re.search(
        r"MERRILL.*?ACCOUNT\s*NO\.?.{0,40}\b([A-Za-z0-9]{2,5}-[A-Za-z0-9]{4,})",
        text,
    ...
    # ==================================================
    fidelity_patterns = [
        r"Account\s*No\.(?:\s*Recipient\s*ID\s*No\.)?\s*([A-Za-z0-9\-]+)",
        r"Account\s*No\.(?:\s*Recipient\s*ID\s*No\.)?\s*([A-Za-z0-9\-]+)",
        r"Account\s*No\.?\s*(?:[:\-]?\s*|\n\s*)(\d{5,})",
...
# Detect 529 college savings plan statements or transaction notices
    ...
    if (
        match_count >= 5
        and "1099-sa" not in lower
        and not any(excl in lower for excl in estimated_exclusions)
    ):
...
# --- STRONG & EARLY 1099-INT DETECTION ---
    ...
    if (
        ...
        return "Expenses", "Property Tax"
    if (
        "washington county dept" in t
        and "bonds and misc taxes" in t
        and "our records indicate that a mortgage company or the" in t
...
EMPLOYER_OVERRIDES = {
    ...
    r"\bINTUIT[, ]+\s*(INC|INCORPORATED|LLC|CORP|CORPORATION)\b": "INTUIT INC",
    r"\bINTUIT\s+(INC|INCORPORATED|LLC|CORP|CORPORATION)\b": "INTUIT INC",
    r"\bNVIDIA[, ]+\s*(CORPORATION|CORP|INC|INCORPORATED)\b": "NVIDIA CORPORATION",
    r"\b(WYZDTA|NVIDIA)\s*,?\s*(CORPORATION|CORP|INC|INCORPORATED)\b": "NVIDIA CORPORATION",
...
def normalize_entity_name(raw: str) -> str:
    ...
    # ----------------------------
    # ----------------------------
    # 10. Remove fuzzy duplicate employer names (OCR-safe)
    # ----------------------------
    tokens = re.split(r"\s{1,}", collapsed)  # split on ANY whitespace
    recombined = []
    ...
    for i, t1 in enumerate(tokens):
        ...
        for j in range(i + 1, len(tokens)):
            if used[j]:
                continue
            if SequenceMatcher(None, t1.lower(), tokens[j].lower()).ratio() > 0.80:
                used[j] = True
        recombined.append(t1)

    collapsed = " ".join(recombined)
    ...
    # ----------------------------
# 12.5 Remove trailing numeric OCR junk (amount fragments)
# ----------------------------
    collapsed = re.sub(
        r'(?:\s+(?:\d+|\.\d+|\d+\.\d+|\d+,\d+))+\s*$',
        '',
    ...
    # ----------------------------
    # 13. Whole-name OCR duplication cleanup
    # ----------------------------
    words = collapsed.split()
    mid = len(words) // 2
    ...
    if SequenceMatcher(None, first.lower(), second.lower()).ratio() > 0.82:
        collapsed = first

    return collapsed or "N/A"


def clean_employer_bookmark(bookmark: str) -> str:
    if not bookmark:
        return bookmark

    # 1️⃣ Hard cut at brackets
    bookmark = re.split(r"[\(\[\{]", bookmark)[0]

    # 2️⃣ Insert spaces between glued uppercase words
    bookmark = re.sub(r"([A-Z])([A-Z]{2,})", r"\1 \2", bookmark)

    tokens = bookmark.split()

    VALID_WORDS = {
        "IBM", "INDIA", "PRIVATE", "LIMITED",
        "INC", "LLC", "LLP", "LTD", "CORP"
    }

    cleaned = []

    for t in tokens:
        # keep valid uppercase words
        if t.upper() in VALID_WORDS:
            cleaned.append(t.upper())
            continue

        # keep long alphabetic words
        if t.isalpha() and len(t) > 3 and t.isupper():
            cleaned.append(t)
            continue

        # otherwise DROP (OCR junk)
        break   # 🚨 stop at first junk tail
...
def parse_w2(text: str) -> Dict[str, str]:
    ...
    INLINE_OVERRIDES = {
        "ACCENTURE LLP": ("accenture llp", "accenture, llp", "accenture,"),
        "FCA US LLC": ("fca us llc", "fca us, llc", "fcaus llc"),
        "FIRESPRING INC": ("firespring inc", "firespring, inc", "firespring,"),
        "PAYCOM PAYROLL LLC": ("paycom payroll llc", "paycom payroll, llc", "paycom payroll,"),
        "NAVKAR TECHNOLOGIES INC": ("navkar technologies inc", "navkar technologies, inc", "navkar technologies,"),
        "MINNESOTA STATE COLLEGES AND UNIVERSITIES": ("minnesota state colleges and universities", "mn state colleges and universities", "mnscu", "minnesota state colleges"),
        


    }

    for canonical, variants in INLINE_OVERRIDES.items():
        if any(v in full_lower for v in variants):
            return {
                'ssn': ssn,
                'ein': ein,
                ...
                for i, line in enumerate(lines):
                    if marker.lower() in line.lower():
                        ...
                        if raw:
                            ...
                            bookmark = emp_name
                        break
    # ---- MAIN EMPLOYER BLOCK (OCR-safe) ----
    for i, line in enumerate(lines):
        low = line.lower()
        if "employer" in low and "address" in low and "zip" in low:
            raw = next_valid_line(lines, i + 1)
            if raw:
                emp_name = normalize_entity_name(raw)
                bookmark = emp_name
    ...
    if emp_name != "N/A":
        ...
        bookmark = clean_employer_bookmark(emp_name)
    print(f"[DEBUG parse_w2 RETURN] bookmark='{bookmark}'")
...
#1099-NEC
def extract_legal_entity_from_page(text: str) -> str | None:
    import re
    ...
    pattern = re.compile(
        r"(.{3,80}?\b(?:LLC|L\.L\.C\.|INC|I\.N\.C\.|CORP|CORPORATION|"
        r"LTD|LIMITED|INCORPORATED))",
        flags=re.IGNORECASE
    )
    ...
    # pick the shortest reasonable name (best OCR signal)
    matches = sorted(m.strip() for m in matches)
    return matches[0]
...
def extract_1099nec_bookmark(text: str) -> str:
    ...
    if not payer_lines:
        legal = extract_legal_entity_from_page(text)
        return legal if legal else "1099-NEC"
...
page_text_cache = {}  # (path, page_index) -> text

def get_text(path, idx):
    key = (path, idx)
    if key not in page_text_cache:
        page_text_cache[key] = extract_text(path, idx)
    return page_text_cache[key]

consolidated_accounts = set()
    ...
    for fname in files:
        ...
        if fname.lower().endswith('.pdf'):
            ...
            for i in range(total):
                ...
                is_acorns = "acorns securities llc" in lower_text
                ...
                # 🚫 HARD BLOCK BAD ISSUERS
                if issuer and issuer.lower() in {
                    "recipient",
                ...
                is_consolidated = (
                    any(k in lower_text for k in consolidated_keywords)
                    or any(a in lower_text and b in lower_text for a, b in consolidated_must_pairs)
                    or any(a in lower_text and b in lower_text and c in lower_text
                           for a, b, c in consolidated_must_triples)
                    # ✅ NEW: Acorns continuation pages
                    or (
                        is_acorns and acct_num and (
                            "summary information" in lower_text
                            or "dividends and distributions" in lower_text
                            or "interest income" in lower_text
                            or "cusip" in lower_text
                        )
                    )
                )
                ...
                if acct_num and (
                    is_consolidated
                    or "acorns securities llc" in text.lower()
        ...
        for form, grp in sorted(groups.items(), key=lambda kv: get_form_priority(kv[0], 'Income')):
            ...
            if form == 'Consolidated-1099':
                ...
                for entry in filtered_grp:
                    ...
                    # 🚫 Never allow issuer-only consolidated bookmarks
                    if not acct:
                        continue

                    forms_label = f"{issuer} – {acct}" if issuer else acct
...
# Append once, with or without bookmark
  ...
  # ✅ get text for this page
                    ...
                    if trustee:
                        ...
                        page_text = get_text(path, idx)  # ✅ get text for this page
                        lbl = extract_1098t_bookmark(page_text)
                elif form == "Child Care Expenses":
                    page_text = get_text(path, idx)  # ✅ get the actual text for this page
