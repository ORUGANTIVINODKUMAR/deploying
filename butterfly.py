...
def extract_account_number(text: str, page_number: int = None) -> str | None:
    """
    Extract and normalize brokerage account numbers safely.
    Designed for OCR-heavy tax documents.
    """

    # --------------------------------------------------
    # 0️⃣ NORMALIZATION (OCR SAFE)
    # --------------------------------------------------
    ...
    # --------------------------------------------------
    # 1️⃣ UNUSED PAGE CHECK
    # --------------------------------------------------


    # --------------------------------------------------
    # 2️⃣ FORM TYPES THAT SHOULD NEVER HAVE ACCOUNT NUMBERS
    # --------------------------------------------------
    skip_keywords = [
        "form 1099-r", "1099 r",
        "form 1098", "1098-t", "1098 t", "1098-e", "1098 e",
        "mortgage interest", "tuition statement",
    ...
    for k in skip_keywords:
        if k in lower:
            if page_number is not None:
                print(f"⛔ Page {page_number} → Skipped ({k})")
            return None

    # --------------------------------------------------
    # 3️⃣ UBS COVER / INSTRUCTION PAGE (SAFE DETECTION)
    # --------------------------------------------------
    if "ubs financial services inc." in lower:
        ...
        if any(t in lower for t in ubs_triggers):
            if page_number is not None:
                print(f"⛔ Page {page_number} → UBS instruction/cover page")
            return None

    # --------------------------------------------------
    # 4️⃣ ACORNS SECURITIES (STRICT & GATED)
    # --------------------------------------------------
    if "acorns securities llc" in lower:
        acorns_patterns = [
            r"Account[\s:\-]*([0-9]{10,}[A-Za-z0-9]*)",
            r"Accou[nmrt]+\s*([0-9]{10,}[A-Za-z0-9]*)",
        ...
        for p in acorns_patterns:
            ...
            if m:
                acc = m.group(1).strip()
                if page_number is not None:
                    print(f"✅ Page {page_number} → ACORNS account: {acc}")
                return acc

    # --------------------------------------------------
    # 5️⃣ STANDARD ACCOUNT PATTERNS (PRIMARY ENGINE)
    # --------------------------------------------------
    std_patterns = [
        r"[A-Za-z]*Account\s*Number\s*[:\-]?\s*([0-9][0-9\s]{5,})",
        r"AccountNumber\s*[:\-]?\s*([0-9][0-9\s]{5,})",
        r"Account\s*No\.?\s*[:\-]?\s*([A-Za-z0-9\-]{4,})",
        r"Account\s*No\.?[^\n\r]*\n\s*([A-Za-z0-9\-]{4,})",
        r"Customer\s*Account\s*Number[:\s]*([A-Za-z0-9\s\-]{4,})",
        r"ORIGINAL:\s*([A-Za-z0-9\s\-]+)",
    ...
    # --------------------------------------------------
    # 6️⃣ APEX CLEARING (INLINE + NEXT LINE)
    # --------------------------------------------------
    apex_next = None
    apex_inline = None
    ...
    if m:
        apex_inline = m.group(1).strip()

    # --------------------------------------------------
    # 7️⃣ COMBINE & RECONCILE (SAFE)
    # --------------------------------------------------
    found = {std_account, apex_next, apex_inline}
    ...
    if found:
        ...
        if len(normalized) == 1:
            ...
            detected_account = std_account or apex_next or apex_inline

    # Fidelity OCR Z-prefix correction
    if detected_account:
        detected_account = re.sub(r"^2Z", "Z", detected_account, flags=re.IGNORECASE)
        detected_account = re.sub(r"^2?24-", "Z24-", detected_account, flags=re.IGNORECASE)
        detected_account = detected_account.replace("–", "-").replace("—", "-")
...
# --- STRONG & EARLY 1099-INT DETECTION ---
    ...
    if (
        "payments received for qualified" in t
        and "this box is reserved for future use" in t
        and "scholarships and grants" in t
        and "checkbox for amounts" in t
        and "academic period beginning" in t
        and "check if a graduate student" in t
...
def extract_1098t_bookmark(text: str) -> str:
    ...
    for line in lines:
        m = re.search(
            r"\b([A-Z][A-Za-z.& ]{3,60}?(University|College|Institute|Academy|Board of Regents))\b",
            line,
            flags=re.IGNORECASE
        )
        if m:
            return normalize_institution(m.group(1))
...
def merge_with_bookmarks(input_dir, output_pdf, meta_json, dummy=""):
   ...
   # remove any zero‐byte files so PdfReader never sees them
    ...
    consolidated_pages = {}  # {(path, i): acct_num}
    ...
    for fname in files:
        ...
        if fname.lower().endswith('.pdf'):
            ...
            for i in range(total):
                ...
                # 🚫 Skip ONLY if this page already assigned to SAME account
                # 🔍 DEBUG — ADD THIS
                print(
                    f"[DEBUG ACCT RAW] {os.path.basename(path)} p{i+1} acct_num={acct_num}",
                    file=sys.stderr
                )
                if acct_num and (path, i) in consolidated_pages:
                    if consolidated_pages[(path, i)] == acct_num:
                        continue
